...
function RouteComponent() {
			...
			<div className="page-wrap px-4 py-10 sm:py-12 lg:py-14">
				<div className="animate-pulse space-y-8">
					<div className="flex flex-wrap items-center justify-between gap-4">
						<div className="h-10 w-44 rounded-full border border-(--line) bg-(--surface-strong)" />
						<div className="h-9 w-36 rounded-full border border-(--line) bg-(--surface-strong)" />
					</div>

					<section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
						<div className="space-y-4">
							<div className="h-5 w-36 rounded-full border border-(--line) bg-(--surface-strong)" />
							<div className="h-14 w-full max-w-2xl rounded-[1.75rem] border border-(--line) bg-(--surface-strong)" />
							<div className="h-8 w-full max-w-xl rounded-3xl border border-(--line) bg-(--surface-strong)" />
						</div>
						<div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
							<div className="h-28 rounded-3xl border border-(--line) bg-(--surface-strong)" />
							<div className="h-28 rounded-3xl border border-(--line) bg-(--surface-strong)" />
							<div className="h-28 rounded-3xl border border-(--line) bg-(--surface-strong)" />
						</div>
					</section>

					<Card className="overflow-hidden rounded-4xl border border-(--line) bg-(--surface-strong) shadow-[0_24px_60px_rgba(30,90,72,0.1)]">
						<CardHeader className="space-y-5 px-6 pt-6 sm:px-8 sm:pt-8">
							<div className="flex items-center gap-3">
								<div className="h-10 w-36 rounded-full border border-(--line) bg-(--surface)" />
								<div className="h-9 w-24 rounded-full border border-(--line) bg-(--surface)" />
							</div>
							<div className="flex flex-col gap-5 sm:flex-row sm:items-center">
								<div className="h-24 w-24 rounded-[1.75rem] border border-(--line) bg-(--surface)" />
								<div className="flex-1 space-y-3">
									<div className="h-11 w-full max-w-sm rounded-2xl border border-(--line) bg-(--surface)" />
									<div className="h-6 w-full max-w-xl rounded-2xl border border-(--line) bg-(--surface)" />
									<div className="flex flex-wrap gap-2">
										<div className="h-8 w-32 rounded-full border border-(--line) bg-(--surface)" />
	...
	if (isError || !data?.student) {
		return (
			<div className="page-wrap px-4 py-10 sm:py-12 lg:py-14">
				<Card className="mx-auto max-w-xl overflow-hidden rounded-4xl border border-destructive/15 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(255,246,246,0.94))] shadow-[0_24px_60px_rgba(150,40,40,0.08)]">
					...
					<CardFooter className="flex justify-center px-6 pb-8 pt-2 sm:px-8">
						<Button
							asChild
							className="h-12 rounded-full bg-(--sea-ink) px-5 text-sm font-semibold text-white shadow-[0_16px_36px_rgba(23,58,64,0.18)] transition hover:bg-(--lagoon-deep)"
						>
		...
		<div className="page-wrap px-4 py-10 sm:py-12 lg:py-14 rise-in">
			<div className="mb-8 flex flex-wrap items-center justify-between gap-4">
				<Button
					asChild
					variant="ghost"
					className="group -ml-2 h-11 gap-2 rounded-full px-3 text-(--sea-ink-soft) transition hover:bg-(--link-bg-hover) hover:text-(--sea-ink)"
				>
					<Link to="/">
				...
				<div className="inline-flex items-center gap-2 rounded-full border border-(--chip-line) bg-(--chip-bg) px-4 py-2 text-[11px] font-bold uppercase tracking-[0.22em] text-(--kicker) shadow-[0_10px_28px_rgba(30,90,72,0.08)]">
			...
			<section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
				<div className="space-y-5">
					...
					<div className="space-y-4">
						<h1 className="display-title max-w-2xl text-4xl font-bold leading-[0.95] tracking-tight text-(--sea-ink) sm:text-5xl lg:text-6xl">
				...
				<div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
					<div className="island-shell rounded-3xl p-5 shadow-[0_12px_28px_rgba(30,90,72,0.08)]">
						...
						<p className="mt-2 font-mono text-sm font-semibold text-(--sea-ink)">
							{student._id}
						</p>
					</div>
					<div className="island-shell rounded-3xl p-5 shadow-[0_12px_28px_rgba(30,90,72,0.08)]">
			...
			<Card className="relative mt-8 overflow-hidden rounded-4xl border border-(--line) bg-(--surface-strong) shadow-[0_24px_60px_rgba(30,90,72,0.1)]">
				<div
					aria-hidden="true"
				...
				<div
					aria-hidden="true"
					className="absolute -right-12 top-0 h-48 w-48 rounded-full bg-(--hero-a) blur-3xl"
				/>
				...
				<CardHeader className="relative space-y-5 px-6 pt-6 sm:px-8 sm:pt-8">
					<div className="flex flex-wrap items-center gap-3">
						...
						<div className="inline-flex items-center gap-2 rounded-full border border-(--line) bg-(--surface) px-3 py-2 text-xs font-semibold text-(--sea-ink-soft) shadow-[0_10px_22px_rgba(30,90,72,0.06)]">
							<Clock3 className="size-4 text-(--lagoon-deep)" />
					...
					<div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
						<div className="flex flex-col gap-5 sm:flex-row sm:items-center">
							<div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-[1.75rem] border border-(--line) bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(236,245,239,0.95))] text-[var(--sea-ink)] shadow-[0_16px_30px_rgba(30,90,72,0.08)] ring-1 ring-white/40">
								<UserRound className="size-12 text-(--lagoon-deep)" />
							</div>

							<div className="space-y-4">
								<CardTitle className="display-title text-3xl font-bold tracking-tight text-(--sea-ink) sm:text-4xl">
									{student.name}
								</CardTitle>
								<CardDescription className="max-w-2xl text-base leading-7 text-(--sea-ink-soft)">
									Dossier personnel centré sur l&apos;essentiel. La lecture
									reste rapide, élégante et confortable sur tous les écrans.
								</CardDescription>

								<div className="flex flex-wrap gap-2">
									<div className="inline-flex items-center gap-2 rounded-full border border-(--line) bg-(--surface) px-3 py-2 text-sm font-medium text-[var(--sea-ink)] shadow-[0_10px_22px_rgba(30,90,72,0.06)]">
										<Hash className="size-3.5 text-(--lagoon-deep)" />
										{student._id}
									</div>
									<div className="inline-flex items-center gap-2 rounded-full border border-(--line) bg-(--surface) px-3 py-2 text-sm font-medium text-[var(--sea-ink)] shadow-[0_10px_22px_rgba(30,90,72,0.06)]">
										<GraduationCap className="size-3.5 text-(--lagoon-deep)" />
										{student.classe}
									</div>
									<div className="inline-flex items-center gap-2 rounded-full border border-(--line) bg-(--surface) px-3 py-2 text-sm font-medium text-[var(--sea-ink)] shadow-[0_10px_22px_rgba(30,90,72,0.06)]">
										<CalendarDays className="size-3.5 text-(--lagoon-deep)" />
										{student.age} ans
									</div>
								</div>
							</div>
						...
						<div className="w-full max-w-sm rounded-3xl border border-(--line) bg-(--surface) p-4 shadow-[0_14px_28px_rgba(30,90,72,0.08)] backdrop-blur">
				...
				<CardContent className="relative px-6 pb-6 pt-4 sm:px-8">
					<div className="grid gap-5 lg:grid-cols-[1fr_0.95fr]">
						<div className="grid gap-5 sm:grid-cols-2">
							<div className="group rounded-3xl border border-(--line) bg-(--surface) p-5 shadow-[0_12px_28px_rgba(30,90,72,0.06)] transition hover:-translate-y-0.5 hover:border-[color-mix(in_oklab,var(--lagoon-deep)_35%,var(--line))] hover:shadow-[0_18px_34px_rgba(30,90,72,0.1)]">
								<div className="flex items-center gap-3">
									<div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-(--sea-ink) text-white shadow-[0_14px_26px_rgba(23,58,64,0.16)] transition group-hover:bg-[var(--lagoon-deep)]">
